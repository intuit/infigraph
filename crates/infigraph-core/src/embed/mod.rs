...
///
/// Activated when `VOYAGE_API_KEY` is set. Uses `voyage-code-3` by default
/// (override with `INFIGRAPH_VOYAGE_MODEL`). Requests 256-dimensional output
/// (Matryoshka truncation) so vectors stay compatible with the local
...
/// Factory: prefer Voyage AI when `VOYAGE_API_KEY` is set, then Model2Vec,
/// otherwise fall back to TrigramEmbedder.
...
/// Create the best available embedder: Voyage AI when `VOYAGE_API_KEY` is set,
/// then Model2Vec if possible, fallback to trigram.
